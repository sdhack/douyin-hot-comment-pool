...
### Changed
- **门槛① 点赞线下调：1000→500**（回复线 50 不变）。联动调整：评论翻页热度早停水位（MC_PAGE_HEAT_FLOOR_LIKES）默认值同步降至 500；视频级 1 万赞门槛不变。
- 存量数据复评实测：500~1000 赞区间仅「养生最好的是少接触人」等少量观点句新晋合格，命中率明显低于头部评论区。

## [0.11.2] - 2026-08-27

### Changed
- **视频级点赞门槛：10000→5000**。搜索结果中 `digg_count`<5000 的视频跳过评论深挖、仅存视频信息；「按赞止停」的截断线同步跟随该值（`MC_STOP_AT_LIKE_FLOOR` 取 `--video-min-likes`）。用途是扩大采样面；历史依据为"命中全部来自万赞以上视频"，放宽后由 AI 判定环节兜底质量。

### Verified
- py_compile 通过；三份离线自检回归通过。

## [0.11.3] - 2026-08-27

### Changed
- **每日入池配额默认值：5→10**（`run_daily.py --quota` 默认；仍为唯一可调运行参数）。配合同日门槛①下调至 500 赞、视频级门槛放宽至 5 千的采样面扩大。

### Verified
- py_compile 与三份离线自检回归通过。
