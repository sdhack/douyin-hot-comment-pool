...
实时采集以前台可轮询方式运行。执行期间每 60 秒汇报一次“本分钟增量 + 累计漏斗 + 当前阶段 + 下一检查点”，并在关键词切换、重试、空结果、入库、清理、配额达标和异常退出时即时汇报。每分钟至少展示视频数、评论数、命中数、已采跳过数、重试数和配额进度；没有新增时必须说明是在请求延时、词间休息、登录/浏览器、接口空桶、重试还是入库等待。阶段和最后进度从 SQLite `batches.status/phase/last_progress_at` 读取，不能只看进程是否存活。
