...
def ensure_mc_client_os_patch(mc_root):
    ...
    try:
        ...
        patched = "import os\n" + src
        open(p, "w", encoding="utf-8", newline="").write(patched)
        return "patched" if re.search(r"(?m)^import\s+os\s*$", open(p, encoding="utf-8").read()) else "verify-failed"
    except OSError:
        return None
...
def ensure_mc_skip_patch(mc_root):
    ...
    try:
        ...
        if method_anchor in src and guard_anchor in src:
            patched = src.replace(method_anchor, method_code + method_anchor, 1)
            patched = patched.replace(guard_anchor, guard_code, 1)
        else:
            # Current MediaCrawler variant has a simpler batch loop without the
            # MC_OPT helper methods. Inject the skip set directly before tasks.
            direct_anchor = "        task_list: List[Task] = []\n"
            direct_insert = (
                "        _skip_path = os.getenv('MC_SKIP_FILE', '')\n"
                "        _skip_ids = set()\n"
                "        if _skip_path and os.path.isfile(_skip_path):\n"
                "            try:\n"
                "                with open(_skip_path, encoding='utf-8') as _f:\n"
                "                    _skip_ids = {line.strip() for line in _f if line.strip()}\n"
                "            except OSError:\n"
                "                pass\n"
                "        aweme_list = [x for x in aweme_list if str(x) not in _skip_ids]\n"
                "        if _skip_ids:\n"
                "            utils.logger.info(f'[MC_OPT] skipped_known_awemes={len(_skip_ids)}')\n"
            )
            if direct_anchor not in src:
                return "no-anchor"
            patched = src.replace(direct_anchor, direct_insert + direct_anchor, 1)
        if patched == src:
            return "verify-failed"
        open(p, "w", encoding="utf-8", newline="").write(patched)
...
def ensure_mc_query_correct_patch(mc_root):
    """让搜索接口支持关闭纠错（幂等，带 .bak 回滚）。

    env MC_QUERY_CORRECT: 1=允许抖音纠错(默认) 0=关闭纠错。
    组合词（如"节气养生"）在网页版搜索返回空时，关闭纠错可能返回原始词结果。
    返回 patched/already/no-anchor/verify-failed/None。
    """
    p = os.path.join(mc_root, "media_platform", "douyin", "client.py")
    if not os.path.isfile(p):
    ...
    try:
        ...
        return None
    if "MC_QUERY_CORRECT" in src:
        return "already"
    try:
        bak = p + ".qcorrect.bak"
        if not os.path.isfile(bak):
            open(bak, "w", encoding="utf-8").write(src)
        anchor = "            'query_correct_type': '1',"
        new = "            'query_correct_type': os.getenv('MC_QUERY_CORRECT', '1'),"
        if anchor not in src:
            return "no-anchor"
        patched = src.replace(anchor, new, 1)
        if patched == src:
        ...
        open(p, "w", encoding="utf-8", newline="").write(patched)
        now = open(p, encoding="utf-8").read()
        return "patched" if "MC_QUERY_CORRECT" in now else "verify-failed"
    except Exception:
        return None
...
def build_cmd(py, mc_main, mode, target, save_dir, get_comment, concurrency, headless, max_n,
    ...
    cmd = [py, mc_main,
           ...
           # per_keyword<=0 视为「不限条数」：上限放大到 1000 以免被 MC 硬编码 15 束缚，
           # 实际由 stop_at_like_floor(视频门槛) 或搜索空页自然结束翻页。
           "--crawler_max_notes_count", str(max_n if max_n and max_n > 0 else 1000)]
    if mode == "search":
        cmd += ["--keywords", target]
    elif mode == "creator":
...
def main():
    ...
    ap.add_argument("--search-sort", dest="search_sort", type=int, default=1, choices=[0, 1, 2],
    ...
    ap.add_argument("--headless", default=False, action=argparse.BooleanOptionalAction,
                    help="默认有头模式（绑定可见浏览器指纹，缓解空响应反爬）。如需后台无痕采集用 --headless 开启")
    ...
    r_os = ensure_mc_client_os_patch(mc_root)
    if r_os == "patched":
        print("[兼容修复] 已补齐 MediaCrawler client.py 的 os 导入，评论热度早停可用")
    elif r_os not in ("already", "not-needed"):
        print(f"[兼容修复] client.py os 导入检查失败({r_os})，可能影响评论抓取")
    ...
    if a.query_correct != 1:
        r_qc = ensure_mc_query_correct_patch(mc_root)
        if r_qc in ("patched", "already"):
            env["MC_QUERY_CORRECT"] = str(a.query_correct)
            print(f"[搜索纠错] {r_qc}; MC_QUERY_CORRECT={a.query_correct}（关闭纠错）")
        else:
            print(f"[搜索纠错] 补丁失败({r_qc})，保持默认纠错")

    # per_keyword<=0（不限条数）时强制启用按赞止停，否则可能无界翻页
