...
- **视频级点赞门槛（默认 5 千）**：搜索结果中 `digg_count`<5000 的视频跳过评论抓取、仅存视频信息——实测命中此前全部来自万赞以上视频；2026-08-27 应用户要求放宽到 5 千以扩大采样面。被门槛跳过的视频不进 skip 名单：**二次遇到时若点赞已涨过门槛会正常抓取**；无统计数据的视频保守放行。可用 `--video-min-likes` 调整。被门槛跳过的视频不进 skip 名单：**二次遇到时若点赞已涨过门槛会正常抓取**；无统计数据的视频保守放行。可用 `--video-min-likes` 调整。
